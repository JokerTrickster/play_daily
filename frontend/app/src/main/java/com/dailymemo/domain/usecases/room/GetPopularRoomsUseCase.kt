...
/**
 * Use case for fetching popular rooms sorted by likes count
 */
class GetPopularRoomsUseCase @Inject constructor(
    private val roomRepository: RoomRepository
) {
    suspend operator fun invoke(page: Int, limit: Int = 20): Result<Pair<List<RoomCardData>, Boolean>> {
        return roomRepository.getPopularRooms(page, limit)
    }
}
